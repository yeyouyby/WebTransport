...
func newCubicSender(
	...
	c := &cubicSender{
		...
		brutal:                     loadBrutalConfig(),
	}
	if c.brutal.enabled {
		minWindow := c.minCongestionWindow()
		c.congestionWindow = brutalWindowBytes(c.brutal.targetBPS, c.brutal.rtt, minWindow)
...
func (c *cubicSender) maybeIncreaseCwnd(
	...
	if c.brutal.enabled {
		target := brutalWindowBytes(c.brutal.targetBPS, c.brutal.rtt, c.minCongestionWindow())
		if target > c.maxCongestionWindow() {
			target = c.maxCongestionWindow()
		}
		c.congestionWindow = target
		c.slowStartThreshold = target
		c.maybeQlogStateChange(qlog.CongestionStateCongestionAvoidance)
		return
	}
